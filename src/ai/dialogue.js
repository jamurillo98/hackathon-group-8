// ─── M4: AI Dialogue Engine ───────────────────────────────────────────────────
// Provides createDialogue() — the core AI/TTS/feedback module.
// Called by the M1 integration shell (src/App.jsx).
// The browser never touches the OpenAI key directly; all API calls go through
// the Vercel serverless function at /api/chat.
// ─────────────────────────────────────────────────────────────────────────────

/**
 * Factory that creates a stateful dialogue session.
 *
 * @param {object} config
 * @param {string} [config.apiUrl="/api/chat"]  - backend proxy endpoint
 * @param {string} [config.model="gpt-4o-mini"] - OpenAI model name
 * @param {string} [config.voiceLang="en-AU"]   - BCP-47 language tag for TTS
 * @returns {{ reply, speak, getFeedback, reset, settings }}
 */
export function createDialogue(config = {}) {
  ...
  // ── 1. THE BRAIN: (scenario + studentSaid) → { reply, mood } ───────────────
  ...
  // ── FALLBACKS (no internet / no backend) ────────────────────────────────────
