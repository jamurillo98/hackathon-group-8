...
// M3: live Three.js figurine scene (replaces FigurineScene placeholder)
...
// Wires M2 UI components, M3 scene, and M4 AI dialogue together.
...
export default function App() {
  ...
  // ── M3: scene state ──────────────────────────────────────────────────────────
  const [clientMood, setClientMood]   = useState('calm')   // drives SceneView mood
  const [isSpeaking, setIsSpeaking]   = useState(false)    // drives SceneView speaking
  ...
  function handleStartSession(scenarioText) {
    ...
    setClientMood('calm')
    setIsSpeaking(false)
    // M4: create fresh dialogue engine for the new scenario
  ...
  // M4: fetch end-of-session feedback when recording stops
      ...
      <div className="mx-auto w-full max-w-5xl flex-1 p-4 lg:p-6 flex flex-col gap-6">
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          ...
          <section className="flex flex-col gap-4" aria-label="Scene and Recording">
            ...
            {/* M3: live Three.js figurine scene */}
            <SceneView
              scenario={scenario}
              mood={clientMood}
              speaking={isSpeaking}
